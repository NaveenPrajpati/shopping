...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("http://localhost:4000/category/create-category", {
        name,
      });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:4000/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
                ...
                <tbody>
                  {categories?.map((c) => (
                      ...
                      <tr key={c._id} className="my-2 border">
                        <td  className="font-semibold p-2">{c.name}</td>
